...
- Skill fs-only 特权脚本 grants 账本 daemon 独占（`~/.pie/grants.json`），扩展零 grant 存储；撤销走设置页「本地打通」→ `skill-grants:revoke` 消息 → daemon 删条目，下次调用重弹授权卡；每次特权脚本执行记一行 `~/.pie/logs/audit.jsonl`。
