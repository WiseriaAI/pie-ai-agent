...
- 模块侧导出：`INTERACTIVE_SELECTOR`（string 常量）、`isVisible`（纯函数）、`ROLE_TO_CN` / `TAG_TO_CN`、状态反射 helper。供 `selector.ts`、类型、单测直接 import 使用。
- 注入侧：因不能 import，仍按既定纪律**内联进** `page-snapshot.ts` / `search-page.ts` / `capture.ts` 三个注入函数。
- 新增 **parity 不变量测试**：断言三处内联副本与权威源逐字/行为一致（守住此次 dedup，防止再次漂移）。

这样既真正统一了"采集页面元素"的判定口径（用户诉求），又不违反 self-contained 注入约束，还顺手消掉现在三份 `INTERACTIVE_SELECTOR`/`isVisible` 的漂移。

### 一条诚实边界（钉死写进 spec）

飞书 / Google Docs 这类 **canvas 编辑器**，用户打字时 light DOM 无事件、无变更（agent 自己也得靠 CDP 键盘注入），因此录制只能靠 keydown 流**尽力**捕获"在该编辑器里输入了内容"这一事实，**不保证还原精确文本**。普通 contenteditable 富文本（评论框、CMS 编辑器等）能通过 `input`/`beforeinput` + textContent 正常采集。
...
- **新增 parity 不变量**：测试断言 `INTERACTIVE_SELECTOR` / `isVisible` 在 page-snapshot / search-page / capture 三处内联副本与 `_shared/interactive.ts` 权威源一致。
