...
| **hover 揭示菜单** | 漏 | **不录原始 hover**（噪声爆炸）；靠"广义 click 现在能录到 `role=menuitem`"。**追加修正（fromPopup 提示）**：当点击落在弹出/下拉容器内（祖先链 12 层内含 `role=menu/listbox/menuitem/option`）时给 action 打 `fromPopup`，serialize 在该步追加"回放前可能需先悬停或点击其触发器展开"——把"LLM 得自己猜有隐藏菜单"变成"明确提示先揭示"。**不**在录制期识别具体触发器（脆），触发器由回放 LLM 在实时页面找（它有 `hover` 工具）。`hover` 仍在 header 工具清单暴露 | 完全不监听 mousemove；仅 role 信号判定（低误报），不识别触发器 |
...
- `RecordedAction` / `CapturedActionPayload` 加 `fromPopup?: boolean`（点击落在弹出菜单/下拉内）；serialize 在该 click 步追加揭示提示。
