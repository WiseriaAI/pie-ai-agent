...
渐进式披露是 agent 的**底层运行时策略，不暴露为用户旋钮**——用户不该、也无需感知"工具是怎么披露的"。因此**没有** `progressiveToolDisclosure` 设置项，也没有"全量披露"回退路径；`seedActiveGroups` 恒为 `core + 环境点亮组`，`selectTools` 恒按 active 组过滤。

> 历史：曾短暂实现过一个默认开的实验开关 + flag-OFF 全量披露基线，真机验收通过后按产品决定移除（不让用户感知）。若日后需要无发版的应急 kill-switch，可重新引入一个 config-store key（非 UI),但当前刻意不保留以免死代码。

`buildActiveGuidanceBlock` 仍保留——它的现役用途是 **resume**：SW 重启续接时 system prompt 用 `resumedActiveToolGroups` 重建,把已加载（lazy/env）组的 guidance 重新内联,避免恢复后只有 schema 没有用法说明。典型新任务的 seed（core + screenshot/skill-mediation,均无 guidance 字段）→ 该块为空,不影响渐进行为。
...
- **resume 引导内联**：`buildActiveGuidanceBlock` 在 startActiveGroups 含 lazy/env 组时内联其 guidance（resume 场景），core-only 时为空。
