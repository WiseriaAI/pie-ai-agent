...
- 实验开关 `progressiveToolDisclosure`（设置页「通用 / 实验」区，默认 **on**）：关 → seed `activeToolGroups` 为全部 group。此时 `buildActiveGuidanceBlock` 把全部 seed-active 组的 guidance **内联回 system prompt**（目录块省略、无激活通知），使工具集 **与引导文本均与今日逐一致**——这是 A/B 可靠性验证的真·基线（否则关闭后会丢失 PDF/scratchpad/skill-authoring 的 prose，污染对照）。flag 开时典型 seed（core + screenshot/skill-mediation，均无 guidance 字段）→ 该块为空，渐进行为不受影响。
