...
import { setSessionAgent, getSessionAgent, createSession } from "./storage";
import {
...
async function seedAgentState(sessionId = SESSION_ID, state: SessionAgentState = freshAgentState()) {
  await setSessionAgent(sessionId, state);
}
...
describe("addPending", () => {
  it("appends to queue and persists", async () => {
    ...
    const state = await getSessionAgent(SESSION_ID);
...
describe("drainPending", () => {
  ...
  it("returns empty array when queue empty (no IDB write)", async () => {
    await seedAgentState();
    const drained = await drainPending(SESSION_ID);
